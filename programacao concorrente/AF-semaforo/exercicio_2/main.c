...
void ponte_entrar(veiculo_t *v) {
    if (v->cabeceira == 0) sem_wait(&sem_continente);
    else sem_wait(&sem_ilha);
}
...
void ponte_sair(veiculo_t *v) {
    ...
    if (count == veiculos_turno) {
        ...
        if (v->cabeceira == 1) {
            for (int i = 0; i < veiculos_turno; i++) sem_post(&sem_ilha);
        }
        else { 
            for (int i = 0; i < veiculos_turno; i++) sem_post(&sem_continente);
        }
        count = 0;
    }
